...
WINTER2025 = ['2025-12-01', '2026-02-28']
SPRING2026 = ['2026-03-01', '2026-05-31']
UPK = 'УПК'
BARISHIHA = 'Барышиха'
...
if menu == "Главная":
    ...
    st.header('Сезон "Весна 2026" - Барышиха/УПК')
if menu == "Барышиха":
    sub_menu = st.sidebar.radio("Барышиха:", ("Главная", "Топ", "Общая стат.", "Личная стат.", "Архив"))
    if sub_menu == "Главная":
        st.title('Весна 2026 (БАРЫШИХА)')
        total_games, total_draws, total_goals, total_assists, total_saves = season_filter(SPRING2026, BARISHIHA)
        ...
        #st.subheader("Перывый гол сезона: Олег Шишкин")
        #st.subheader("Рыжий пришел на футбол: 1")
        st.write("© MitinoSarayTeam")
    elif sub_menu == "Топ":
        st.title('Весна 2026 (БАРЫШИХА)')
        players_df = season_filter_players_stats(SPRING2026, 2, BARISHIHA)
        ...
        st.title('Весна 2026 (БАРЫШИХА)')
        players_df = season_filter_players_stats(SPRING2026, 2, BARISHIHA)
        stats_df = season_filter_stats(SPRING2026, BARISHIHA)
        ...
        stat_submenu = st.sidebar.radio("Архив данных:", ("Лето 2025", "Осень 2025"))
        ...
        if stat_submenu == "Осень 2025":
            st.title('Осень 2025')
            player_submenu = st.sidebar.radio("Статистика за осень 2025:", ("Главная", "Топ", "Общая стат.", "Личная стат."))
            if player_submenu == "Главная":

#        elif stat_submenu == "Осень 2025":
#            st.title('Осень 2025')
#            pl_submenu = st.sidebar.radio("Барышиха:", ("Главная", "Топ", "Общая стат.", "Личная стат.", "Архив"))
#            if pl_submenu == "Главная":
...
if menu == "УПК":
    ...
    if sub_menu == "Главная":
        ...
        st.subheader(f"Пасов отдано: {total_assists}")
        st.subheader(f"Shutout: {total_saves}")
        st.subheader("Первый гол сезона: Олег Шишкин")
        st.subheader("Рыжий пришел на футбол: 1")
        st.write("© MitinoSarayTeam")
    elif sub_menu == "Топ":
        ...
        stat_submenu = st.sidebar.radio("Архив данных:", ("Осень 2025", "Зима 2025"))
        if stat_submenu == "Осень 2025":
            st.title('Осень 2025')
            player_submenu = st.sidebar.radio("Статистика за осень 2025:", ("Главная", "Топ", "Общая стат.", "Личная стат."))
            if player_submenu == "Главная":
        ...
        elif stat_submenu == "Зима 2025":
            st.title('Зима 2025')
            player_submenu = st.sidebar.radio("Статистика за Зиму 2025:", ("Главная", "Топ", "Общая стат.", "Личная стат."))
            if player_submenu == "Главная":
                total_games, total_draws, total_goals, total_assists, total_saves = season_filter(WINTER2025, UPK)
                st.subheader(f"Игр сыграно: {total_games} (из них ничьи: {total_draws})")
                st.subheader(f"Голов забито: {total_goals}")
                st.subheader(f"Пасов отдано: {total_assists}")
                st.subheader(f"Shutout: {total_saves}")
                ...
                players_df = season_filter_players_stats(WINTER2025, 2, UPK)
                points_df = players_df[['player_name', 'points']].sort_values(by='points', ascending=False)
                points_df = points_df.rename(columns={'player_name': 'Игрок', 'points': 'Очки'})
                st.subheader('Общий рейтинг')
                st.dataframe(points_df, hide_index=True, height=200, width=400)
                ...
                players_df = season_filter_players_stats(WINTER2025, 2, UPK)
                stats_df = season_filter_stats(WINTER2025, UPK)
